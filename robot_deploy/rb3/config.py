...
#
# Fitted over 0.66-2.22 m (6 readings): the model reads ~1.87x TOO FAR, and the
# bias is multiplicative — scale-only fits at 0.091 m RMS against 0.401 m for
# offset-only, and affine buys nothing (0.089 m) so the second term stays at 0.
#
...
# indistinguishable; that was an artefact of the narrow range, not a property of
# the model. Hence the coverage rule in --fit.
#
# DO NOT expect better than this. The per-reading ratio scatters +-9% (2.10 down
# to 1.61), and that is the model, not the measurements: monocular depth on a
# wall is inferred from the room around it — floor and ceiling lines, skirting,
# sockets, texture falloff — so the answer shifts with what else is in frame.
# Residuals after correction run to +-13%. Treat depth as good to roughly 10%
# and leave obstacle margins accordingly.
#
# Calibrated in the room the robot works in, deliberately: the correction is
# scene-dependent, so a tidier blank-wall setup would be both harder for the
# model and less representative of what it will actually see.
DEPTH_SCALE    = 0.5425
DEPTH_OFFSET   = 0.0
